...
| `audit_application_images` | Audits WorkSpaces Applications (AppStream 2.0) **images, image builders, and app-block builders** — stale base images, pinned/old agents, errored/SHARED images, and **builders left RUNNING**, each with its **actual regional $/hr and $/month list rate** (Tier 0; rates need Tier 1). |
...
| `get_euc_cost_forecast` | **Forecast upcoming EUC spend** via Cost Explorer's forecasting model — mean total + per-period values with an 80% prediction interval, filtered to the EUC services discovered in recent actual spend. Reports the recent vs trailing daily run-rate and **flags when a hot week (e.g. running image builders) is inflating the forecast** (Tier 1). |
...
| `get_euc_audit_trail` | **"Who changed what"** — recent EUC management events from CloudTrail (last 90 days, no trail required); mutations-only by default, flags destructive actions and errors (e.g. AccessDenied). Single-service queries also scan **curated mutation event names across the full window** (immune to account write-noise), and every response reports **honest window coverage** (`window_fully_covered` / `scanned_back_to`) (Tier 0). |
...
| `get_euc_service_prices` | **Authoritative AWS list prices** for any EUC resource in a region (AWS Price List): Applications $/hr by instance type/function/**license model (BYOL-aware)**, Secure Browser $/monthly-active-user by tier, Core managed-instance fee SKUs (one per billing option), and Personal bundle rates — with a **near-miss listing of the storage pairings AWS does price** when the exact configuration has no SKU — so cost answers never come from model memory. Applications pricing includes the **On-Demand stopped-instance fee** and per-fleet-type billing guidance (Always-On bills 24/7; On-Demand bills only streaming hours + ~$0.025/hr idle), and accepts a **`fleet_name`** to auto-resolve instance type, capacity, and the platform from the fleet's image (**BYOL-aware** — catches Windows 10/11 fleets that the fleet API doesn't label) with idle/provisioned monthly estimates. Personal bundle pricing is **license-aware** (Windows 10/11 = BYOL; Amazon Linux/Ubuntu/RHEL/Rocky all resolve with their real license models), covers **every storage variant** (the suffixed variant bundles AWS hides them in) and all bundle families incl. Graphics/GeneralPurpose, and includes the **AutoStop-vs-AlwaysOn break-even hours/month**. Also prices **WorkSpaces Pools** (streaming $/hr Included vs BYOL + stopped fee + user fee) and surfaces the **Microsoft user fees** (RDS SAL) that Included-license Windows fleets add. The full empirical model of the Price List for EUC is documented in [`docs/pricing-api.md`](docs/pricing-api.md). Supports **multi-region comparison** in one call (`regions`), and every no-match/partial-match response lists the **valid alternatives AWS does price** (storage pairings, OS variants) instead of bare nulls (Tier 1). |
...
> **Pricing estimates are advisory.** Rates come from the AWS Price List API — public **list**
> prices; private pricing, EDP/PPA discounts, and credits are not reflected — and AI assistants can
> occasionally construct pricing queries imperfectly. Treat estimates as directional and verify
> real spend with the cost tools (Cost Explorer actuals).

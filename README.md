...
| `get_euc_service_prices` | **Authoritative AWS list prices** for any EUC resource in a region (AWS Price List): Applications $/hr by instance type/function/**license model (BYOL-aware)**, Secure Browser $/monthly-active-user by tier, Core managed-instance fee SKUs (one per billing option), and Personal bundle rates — with a **near-miss listing of the storage pairings AWS does price** when the exact configuration has no SKU — so cost answers never come from model memory (Tier 1). |

...
| `get_euc_service_prices` | Authoritative Price List rates for the whole portfolio (Applications hourly incl. BYOL + stopped-instance fee + Microsoft user fees + `fleet_name` auto-resolution incl. platform-from-image; **Pools** streaming/stopped/user-fee rates; Secure Browser MAU tiers; Core fee SKUs per billing option; Personal bundles license-aware across all OSes + storage-variant bundles + AutoStop break-even hours; multi-region comparison; no-match responses list valid alternatives; explicit list-price assumptions). Ground truth: `docs/pricing-api.md` | `pricing:GetProducts` (+ `appstream:DescribeFleets`/`DescribeImages` for fleet_name) |
